...
public class StatementServiceImpl implements StatementService {
    ...
    public byte[] generateStatement(String numeroCompte, LocalDate debut, LocalDate fin) {
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            throw new RuntimeException("Erreur lors de la génération du relevé de compte", e);
        }
    }
    ...
    private void addTransactionsTable(Document document, List<Transaction> transactions) {
        ...
        if (transactions.isEmpty()) {
            ...
            for (Transaction t : transactions) {
                ...
                table.addCell(createCell(t.getSoldeAvant() != null ? t.getSoldeAvant().toString() : "-"));
                table.addCell(createCell(t.getSoldeApres() != null ? t.getSoldeApres().toString() : "-"));
            }
        }
    ...
    private void addSummary(Document document, Account account, List<Transaction> transactions) {
        ...
        summaryTable
                .addCell(new Cell().add(new Paragraph("Nombre d'opérations:").setBold()).setBorder(Border.NO_BORDER));
        summaryTable.addCell(new Cell().add(new Paragraph(String.valueOf(transactions.size())))
                .setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.RIGHT));

        summaryTable.addCell(new Cell().add(new Paragraph("Solde actuel:").setBold()).setBorder(Border.NO_BORDER));
